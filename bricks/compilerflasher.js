compilerflasher = function(lf){
    ...
    this.pluginHandler = new function(){
        ...
        this.clickedPort = function()
        {
            ...
            $.ajax({url: url, contentType: "text/plain", xhrFields: {withCredentials: false}});//$.get(url);
                    ...
                    else
                    {
                        ...
                        if (typeof(document.getElementById('plugin0').getPorts) !== "undefined")
                        {
                            ...
                            if (oldSerialPortsAvail != serialPortsAvail)
                            {
                                ...
                                url = url.replace("PLUGIN_META", JSON.stringify({ "success": true, "plugin" : true, "version": document.getElementById('plugin0').version, "probeUSBports" : oldPortsAvail, "serialLibPorts" : parsedList}) );
                                $.ajax({url: url, contentType: "text/plain", xhrFields: {withCredentials: false}});//$.get(url);
        ...
        this.addTo = function(where, pluginUrl)
        {
            ...
            {
                $.ajax({url: url, data: "text/plain"}).done(function(){
...
//        alertElement += "<button type='button' class='close' data-dismiss='alert'>x</button>";
    ...
    if($("select#cb_cf_boards").length > 0)
    {
        ...
        $.ajax({
            dataType: "json",
            contentType: "text/plain", xhrFields: {withCredentials: false},
            url: "https\x3A\x2F\x2Fcodebender.cc\x2Fboard\x2Flistboards",
            success: function(data){console.log("data =", data);boardsListCallback(data)}
    ...
    if($("select#cb_cf_programmers").length > 0)
    {
        ...
        $.ajax({
            dataType: "json",
            url: "https\x3A\x2F\x2Fcodebender.cc\x2Fboard\x2Fprogrammers",
            contentType: "text/plain", xhrFields: {withCredentials: false},
    ...
    this.burn_bootloader = function() {
        ...
        if(this.pluginHandler.canBurnBootloader(this.selectedProgrammer))
        {
            ...
            {
                var cb = this;
                $.ajax({url: "\x2Fbootloader\x2F" + this.selectedBoard['bootloader']['file'].replace(".hex", ".txt"), contentType: "text/plain", xhrFields: {withCredentials: false}})
